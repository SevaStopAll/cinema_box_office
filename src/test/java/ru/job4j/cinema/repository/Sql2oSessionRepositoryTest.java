...
class Sql2oSessionRepositoryTest {
    ...
    public void clearFilmSessions() {
        var filmSessions = sql2oSessionRepository.findAll();
        for (var filmSession : filmSessions) {
            sql2oSessionRepository.deleteById(filmSession.getId());
        }
    }
    ...
    public void whenSaveThenGetSame() {
        ...
        var filmSession = sql2oSessionRepository.save(new Session(1, film.getId(), hall.getId(), startTime, endTime, 300));
        var savedFilmSession = sql2oSessionRepository.findById(filmSession.getId()).get();
    ...
    public void whenSaveSeveralThenGetAll() {
        var startTime = now().truncatedTo(ChronoUnit.MINUTES);
        var endTime = startTime.plusMinutes(90).truncatedTo(ChronoUnit.MINUTES);
        var filmSession1 = sql2oSessionRepository.save(new Session(1, film.getId(), hall.getId(), startTime, endTime, 300));
        var filmSession2 = sql2oSessionRepository.save(new Session(2, film.getId(), hall.getId(), startTime, endTime, 300));
        var filmSession3 = sql2oSessionRepository.save(new Session(3, film.getId(), hall.getId(), startTime, endTime, 300));
        var result = sql2oSessionRepository.findAll();
        assertThat(result).isEqualTo(List.of(filmSession1, filmSession2, filmSession3));
    }

    @Test
    public void whenDontSaveThenNothingFound() {
        assertThat(sql2oSessionRepository.findAll()).isEqualTo(emptyList());
        assertThat(sql2oSessionRepository.findById(0)).isEqualTo(empty());
    }
    ...
    public void whenDeleteThenGetEmptyOptional() {
        ...
        var endTime = startTime.plusMinutes(90).truncatedTo(ChronoUnit.MINUTES);
        var filmSession = sql2oSessionRepository.save(new Session(1, film.getId(), hall.getId(), startTime, endTime, 300));
        var isDeleted = sql2oSessionRepository.deleteById(filmSession.getId());
        var savedFilmSession = sql2oSessionRepository.findById(filmSession.getId());
    ...
    public void whenDeleteByInvalidIdThenGetFalse() {
        assertThat(sql2oSessionRepository.deleteById(0)).isFalse();
    }
}